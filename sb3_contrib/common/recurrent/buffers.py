...
class GraphRecurrentRolloutBuffer(RecurrentRolloutBuffer):
    def __init__(
        self,
        buffer_size: int,
    ...
    def _get_samples(
        ...
        if isinstance(self.observation_space, spaces.Graph):
            ...
            data_segments = self.seq_start_indices.tolist()
            data_segments.append(batch_inds.shape[0])
            for idx in range(len(data_segments)-1):
                start_idx = data_segments[idx]
        ...
        return RecurrentRolloutBufferSamples(
            # (batch_size, obs_dim) -> (n_seq, max_length, obs_dim) -> (n_seq * max_length, obs_dim)
            observations=self.to_torch(selected_observations),
            actions=self.pad(self.actions[batch_inds]).reshape((padded_batch_size,) + self.actions.shape[1:]),
            old_values=self.pad_and_flatten(self.values[batch_inds]),
            old_log_prob=self.pad_and_flatten(self.log_probs[batch_inds]),
            advantages=self.pad_and_flatten(self.advantages[batch_inds]),
